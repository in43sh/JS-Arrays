...
function finder (arr) {
  ...
  for (var i = 0; i < arr.length; i++) {
    if (arr[i] === randomNum) {return true}
  }
  return false
}
...
function addItem (myGroceryList, item) {
  // return myGroceryList.push(item) // not working
...
function maker () {
  var arr = []
  for (var i = 1; i < 216; i++) {arr.push(i)}
  return arr
}
...
function addTen(numbers) {
  for (var i = 0; i < numbers[i]; i++) {
    if (typeof numbers[i] === 'string') {numbers [i] = parseInt(numbers[i], 10)}
    numbers[i] += 10
  }
  return numbers
}
